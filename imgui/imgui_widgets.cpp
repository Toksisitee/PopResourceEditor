...
const ImVec4 k_EditorScalarValueColor = ImVec4( 0.80f, 0.81f, 0.57f, 1.00f );
...
size_t g_ImEditorFlags = g_ImEditorFlags;

void ImGui::Editor::PushFlags( ImGui_EditorFlags_ uFlag )
{
	g_ImEditorFlags |= uFlag;
}

void ImGui::Editor::PopFlags()
{
	g_ImEditorFlags = ImEditorFlags_None;
}

#if EDITOR_IMGUI
ImU32 ImGui::Editor::InterporlateColor( float& fTime, const float& fDuration, const ImVec4& vStartColor, const ImVec4& vEndColor )
{
	float t = fmodf( fTime, 2.0f * fDuration ) / fDuration;

	if ( t > 1.0f ) {
		t = 2.0f - t;
	}

	ImVec4 vInterpolatedColor;
	vInterpolatedColor.x = vStartColor.x + (vEndColor.x - vStartColor.x) * t;
	vInterpolatedColor.y = vStartColor.y + (vEndColor.y - vStartColor.y) * t;
	vInterpolatedColor.z = vStartColor.z + (vEndColor.z - vStartColor.z) * t;
	vInterpolatedColor.w = 1.0f; 

	fTime += ImGui::GetIO().DeltaTime;
	return ImGui::ColorConvertFloat4ToU32( vInterpolatedColor );
}
#endif
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags )
{
	...
	const float k_fDuration = 1.0f;
	ImU32 col = GetColorU32( (held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button );
	if ( hovered ) {
		const ImVec4 startColor = ImGui::GetStyleColorVec4( ImGuiCol_ButtonActive );
		const ImVec4 endColor = ImGui::GetStyleColorVec4( ImGuiCol_ButtonHovered );
		col = ImGui::Editor::InterporlateColor( s_fTime, k_fDuration, startColor, endColor );
	}
#else
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags )
{
...
#if EDITOR_IMGUI
	// Hacky fix to inappropriate control sizes, e.g. overlapping other controls due to padding
	if ( g_ImEditorFlags & ImEditorFlags_NewSlider )
	...
	if ( g_ImEditorFlags & ImEditorFlags_NewSlider ) {
		...
		w = IM_TRUNC( ImMax( 1.0f, region_max_x - window->DC.CursorPos.x ) );
	}
	else
		w = CalcItemWidth();
#else
	...
	if ( g_ImEditorFlags & ImEditorFlags_NewSlider ) {
		RenderNavHighlight( frame_bb, id );
		float fCentreY = (frame_bb.Min.y + frame_bb.Max.y) * 0.5f;
		// Draw "filled" or "active" bar up to the circle position
		{
			const ImU32 frame_col = GetColorU32( ImGuiCol_FrameBgActive );
			const ImVec2 vStartPos = ImVec2( frame_bb.Min.x, fCentreY );
			const ImVec2 vEndPos = ImVec2( grab_bb.Min.x, fCentreY );
			window->DrawList->AddLine( vStartPos, vEndPos, frame_col, 6.0f );
		}
		// Draw the rest of the "inactive" or "unfilled" bar
		{
			ImU32 frame_col = GetColorU32( ImGuiCol_FrameBgHovered );
			if ( hovered ) {
				static float s_fTime = 0.0f;
				const float k_fDuration = 1.0f;

				const ImVec4 startColor = ImGui::GetStyleColorVec4( ImGuiCol_FrameBgActive );
				const ImVec4 endColor = ImGui::GetStyleColorVec4( ImGuiCol_FrameBgHovered );

				frame_col = ImGui::Editor::InterporlateColor( s_fTime, k_fDuration, startColor, endColor );
			}
			const ImVec2 vStartPos = ImVec2( grab_bb.Min.x, fCentreY );
			const ImVec2 vEndPos = ImVec2( frame_bb.Max.x, fCentreY );
			window->DrawList->AddLine( vStartPos, vEndPos, frame_col, 6.0f );
		}
	}
	else {
...
#if EDITOR_IMGUI
		if ( g_ImEditorFlags & ImEditorFlags_NewSlider ) {
			const float fCircleRad = ImMax( 2.0f, g.FontSize * 0.5f + 1.0f );
			const ImU32 uCol = GetColorU32( g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab );
			const ImVec2 vPos = ImVec2( grab_bb.Min.x, (frame_bb.Min.y + frame_bb.Max.y) * 0.5f );
			window->DrawList->AddCircleFilled( vPos, fCircleRad, uCol );
		}
		else {
...
#if EDITOR_IMGUI
	if ( g_ImEditorFlags & ImEditorFlags_NewSlider ) {
		//RenderTextClipped( ImVec2( w, frame_bb.Min.y - label_size.y ), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2( 0.5f, 0.5f ) );
		const ImVec2 value_size = CalcTextSize( value_buf, value_buf_end, true );
		PushStyleColor( ImGuiCol_Text, k_EditorScalarValueColor );
		RenderText( ImVec2( frame_bb.Max.x - value_size.x, frame_bb.Min.y - label_size.y ), value_buf );
		PopStyleColor();
	}
	else {
		RenderTextClipped( frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2( 0.5f, 0.5f ) );
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
	...
	if ( hovered ) {
		...
		const ImVec4 startColor = ImGui::GetStyleColorVec4( ImGuiCol_HeaderActive );
		const ImVec4 endColor = ImGui::GetStyleColorVec4( ImGuiCol_HeaderHovered );

		float t = fmodf( s_fTime, 2.0f * k_fDuration ) / k_fDuration;
		...
		ImVec4 finalClr;
		finalClr.x = startColor.x + (endColor.x - startColor.x) * t;
		finalClr.y = startColor.y + (endColor.y - startColor.y) * t;
		finalClr.z = startColor.z + (endColor.z - startColor.z) * t;
		finalClr.w = 1.0f;

		const ImU32 col = ImGui::ColorConvertFloat4ToU32( finalClr );
		RenderFrame( bb.Min, bb.Max, col, false, 0.0f );

		s_fTime += ImGui::GetIO().DeltaTime;
...
#if EDITOR_IMGUI
	if ( g_ImEditorFlags & ImEditorFlags_NewTabs ) {
		const float fCircleRad = ImMax( 0.2f, g.FontSize * 0.4f + 0.2f );
		const ImGuiWindow* window = GetCurrentWindow();
		window->DrawList->AddCircleFilled( ImVec2( text_ellipsis_clip_bb.Min.x, text_pixel_clip_bb.GetCenter().y - (fCircleRad / 2) ), fCircleRad, GetColorU32( ImGuiCol_Text ), 12 );
		// Donț think this works when the font size changes!
		text_ellipsis_clip_bb.Min.x += (fCircleRad * 3);
		text_ellipsis_clip_bb.Max.x += (fCircleRad * 3);
		text_pixel_clip_bb.Max.x += (fCircleRad * 3);
		ellipsis_max_x += (fCircleRad * 3);
	}
#endif
