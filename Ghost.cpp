...
namespace Assets
{
	...
	Result CGhost::Export( std::string& sFilePath )
	{
		...
		BMP BMP;
		size_t uIndex = 0;
		auto pColorTable = m_Palette.GetPalette();
	...
	void CGhost::GenerateTable( uint8_t uOpacity )
	{
		...
		RGB* pColorPalette = m_Palette.GetPalette();
		uint8_t* pData = &m_Data[0];
		RGB blend;

		for ( uint32_t y = 0; y < k_uHeight; ++y ) {
			for ( uint32_t x = 0; x < k_uWidth; ++x ) {
				const RGB& base = pColorPalette[y % Palette::k_uNumColors];
				const RGB& target = pColorPalette[x % Palette::k_uNumColors];
